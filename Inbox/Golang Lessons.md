...
> 1. Oбъем памяти, который нам нужно выделить 2. Что эта память представляет
...
В Go есть "Конверсия" (Conversioт - преобразование типа).**
...
Какого её **zero value**? Сколько памяти выделяется?
* bool - 1 byte
* int16 - 2 byte
* float32 - 4 byte
  ~~**7 byte?**~~  -> **8 byte**
...
1. Пакет должен быть целенаправленный. Пакет должен обеспечивать, а не содержать.
...
2. Чтобы пакеты были портативными, они должны быть разработаны с учетом возможности повторного использования.
...
1. Позволяет разработчику определить, где находится пакет внутри проекта GO.
2. Позволяет определять, что это за проект GO и как структурирован этот проект.
3. Улучшает общение между членами команды и способствует чистому дизайну пакетов и архитектуре проекта, которую можно обсуждать.
...
1. Используйте каналы для оркестровки и координации горутин. 
